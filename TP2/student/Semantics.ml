...
(* ...............A COMPLETER .......................................*)
ruleRead _env _expr mem = let (expr_addr,mem1) = value_of_expr (_expr, mem) _env in
          match expr_addr with
          | (ErrorValue _) as result -> (result,mem1)
          | (ReferenceValue addr) -> (match lookforMem addr mem1 with
            |NotFound -> (ErrorValue(UnknownReferenceError addr), mem1)
            |Found value -> (value, mem1))
...
ruleWrite _env _refexpr _valexpr mem = let (val2, mem2) = value_of_expr (_valexpr, mem) _env in
          match val2 with
            | (ErrorValue _) as result -> (result,mem2)
            | _ -> let (expr_addr,mem3) = value_of_expr (_refexpr, mem2) _env in
              match expr_addr with
                | (ErrorValue _) as result -> (result,mem3)
                | (ReferenceValue addr) -> (NullValue,(addr,val2)::mem3)
...
ruleSequence _env _left _right mem = let (val1, mem2) = value_of_expr (_left, mem) _env in
        match val1 with
          | (ErrorValue _) as result -> (result,mem2)
          | NullValue -> let (val2, mem3) = value_of_expr (_right, mem2) _env in
              (match val2 with
              | (ErrorValue _) as result -> (result,mem3)
              | _ -> (val2, mem3))
...
ruleWhile _env _cond _body mem = let (res, mem2) = value_of_expr (_cond, mem) _env in
          match res with
            | (ErrorValue _) as result -> (result,mem2)
            | BooleanValue(false) -> (NullValue, mem2)
            | BooleanValue(true) -> let (corps, mem3) = value_of_expr (_body, mem2) _env in
                (match corps with
                  | (ErrorValue _) as result -> (result,mem3)
                  | _ -> ruleWhile _env _cond _body mem3)
            | _-> (ErrorValue(TypeMismatchError),mem2)
...
ruleReference _env _expr mem = let (val_expr,mem2) = value_of_expr (_expr,mem) _env in
          match val_expr with
            | (ErrorValue _) as result -> (result,mem2)
            | _ -> let addr = newReference() in (ReferenceValue(addr), (addr,val_expr)::mem2)
(* ...............A COMPLETER .......................................*)
